...
const Tickets = () => {
  ...
  const getCleanEventName = (ticket, eventMap, bookingDetailsMap) => {
    ...
    try {
      if (eventId) {
        ...
        xhr.open('GET', `http://localhost:5001/api/events/${eventId}`, false); // false for synchronous
        xhr.send();
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5001/api/auth/profile', {
      ...
      .then(userData => {
        ...
        return fetch('http://localhost:5001/api/ticketorders', {
          headers: { 
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
      ...
      .then(({ ticketData, userId }) => {
        ...
        const fetchBookingDetails = async () => {
          ...
          try {
            ...
            const allBookingsResponse = await fetch(`http://localhost:5001/api/bookings`, {
              headers: { 
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
              }
        ...
        const processTicketsWithBookings = async () => {
          ...
          try {
            const allEventsResponse = await fetch(`http://localhost:5001/api/events`, {
              headers: { 
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
              }
  ...
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (!token) return;
    fetch('http://localhost:5001/api/shopOrders', { headers: { 'Authorization': `Bearer ${token}` } })
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5001/api/ticketorders/${transferModal.ticket._id || transferModal.ticket.id}/transfer`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleCancel = async (ticket) => {
    ...
    try {
      // Otherwise, use the API
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5001/api/ticketorders/${ticket._id || ticket.id}/cancel`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleDelete = async (ticket) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5001/api/ticketorders/${ticket._id || ticket.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
