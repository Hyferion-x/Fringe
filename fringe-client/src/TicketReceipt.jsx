...
const TicketReceipt = () => {
  ...
  useEffect(() => {
    ...
    // Fetch ticket order
    fetch(`http://localhost:5001/api/ticketOrders`, {
      ...
      .then(async data => {
        if (data && Array.isArray(data)) {
          ...
          if (found && found.bookingId) {
            // Fetch booking
            const bookingRes = await fetch(`http://localhost:5001/api/bookings/${found.bookingId}`, {
              headers: { 'Authorization': `Bearer ${token}` }
            });
            if (bookingRes.ok) {
              ...
              if (bookingData.eventId) {
                // Fetch event
                const eventRes = await fetch(`http://localhost:5001/api/events/${bookingData.eventId}`);
                if (eventRes.ok) {
