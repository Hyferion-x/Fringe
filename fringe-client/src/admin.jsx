...
const Admin = () => {
  ...
  useEffect(() => {
    if (activeTab === 'customers') {
      ...
      const fetchAllShopOrders = async () => {
        const token = localStorage.getItem('token');
        try {
          const res = await fetch(`${API_URL}/shopOrders`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  useEffect(() => {
    ...
    setOrdersError('');
    const token = localStorage.getItem('token');
    fetch(`http://localhost:5001/api/ticketOrders/admin?userId=${selectedOrdersUser._id}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : [])
      .then((tickets) => {
  ...
  const handleEventSubmit = async (e) => {
    ...
    try {
      ...
      if (editingEvent) {
        // Edit
        res = await fetch(`${API_URL}/events/${editingEvent._id}`, {
          method: 'PUT',
          headers: {
  ...
  async function handleCancelTicket(row) {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/ticketorders/${row.ticketOrderId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!res.ok) {
  ...
  const markAsUnread = async (id) => {
    const token = localStorage.getItem('token');
    await fetch(`http://localhost:5001/api/notifications/${id}/unread`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${token}` }
    });
    fetchNotifications();
  ...
  const openCustomerModal = async (customer) => {
    ...
    try {
      ...
      if (!orders.length) {
        const res = await fetch(`${API_URL}/ticketorders`, { headers: { Authorization: `Bearer ${token}` } });
      ...
      const shopRes = await fetch(`${API_URL}/shopOrders`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleMerchSubmit = async (e) => {
    ...
    try {
      ...
      if (editingMerch) {
        // Edit
        res = await fetch(`${API_URL}/merchandise/${editingMerch._id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        // Add
        res = await fetch(`${API_URL}/merchandise`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  const handleDeleteMerch = async (itemId) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/merchandise/${itemId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (!res.ok) {
  ...
  useEffect(() => {
    const fetchAllExpectedSales = async () => {
      ...
      await Promise.all(events.map(async (event) => {
        try {
          const res = await fetch(`http://localhost:5001/api/events/expected-sales/${event._id}`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          if (res.ok) {
            const data = await res.json();
  ...
  const handleSaveExpectedSales = async (eventId) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`http://localhost:5001/api/events/expected-sales/${eventId}`, {
        method: 'PUT',
  ...
  const fetchInbox = async () => {
    ...
    const token = localStorage.getItem('token');
    try {
      const url = `${API_URL}/contactMessages`;
  ...
  const handleMarkRead = async (msg, isRead) => {
    const token = localStorage.getItem('token');
    await fetch(`http://localhost:5001/api/events/contact-messages/${msg._id}/read`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const handleDeleteMessage = async (msg) => {
    const token = localStorage.getItem('token');
    await fetch(`http://localhost:5001/api/events/contact-messages/${msg._id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  useEffect(() => {
    if (activeTab === 'merchorders') {
      const fetchAllShopOrders = async () => {
        const token = localStorage.getItem('token');
        try {
          const res = await fetch(`${API_URL}/shopOrders/all`, { headers: { Authorization: `Bearer ${token}` } });
          const data = res.ok ? await res.json() : [];
          setAllShopOrders(data);
        } catch {
          setAllShopOrders([]);
